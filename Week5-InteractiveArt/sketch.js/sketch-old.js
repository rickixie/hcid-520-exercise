...
function changeAllColor(){
    ...
    var s = random(100);  
    var l = random(30, 100);//avoid the high saturation
    var a = random(0.5,1); 
...
function Ball() {
  ...
  var rx = random(diameter, width-diameter);
  var ry = random(diameter, height-diameter);
  ...
  if (((rx + vx) > width - radius) || ((rx) < radius)) {
    vx = -vx;
    if(soundKey!=0){
      playSound(soundKey-1);
    }
   
  }
  if (((ry + vy) > height - diameter/2) || ((ry + vy) < radius)) { 
    vy = -vy;
     if(soundKey!=0){
       playSound(soundKey-1);
    }
